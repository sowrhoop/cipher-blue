...
if command -v dnf5 >/dev/null 2>&1; then
  dnf5 config-manager addrepo --from-repofile="https://repo.secureblue.dev/secureblue.repo" || true
  rm -f /etc/dnf/protected.d/sudo.conf || true

  # Create an array of packages to remove
  mapfile -t PACKAGES_TO_REMOVE <<EOF
...
EOF
  ...
  # Iterate over each package to log and remove it individually
  ...
  dnf5 install --setopt=install_weak_deps=False nautilus trivalent tlp fapolicyd unbound dnscrypt-proxy ima-evm-utils keyutils openssl -y || true
fi
